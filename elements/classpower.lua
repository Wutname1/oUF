...
local function Update(self, event, unit, powerType)
	if
		(not (unit and
			(UnitIsUnit(unit, 'player') and (not powerType or powerType == ClassPowerType) or
				unit == 'vehicle' and powerType == 'COMBO_POINTS')))
	 then
		return
	end
...
do
	...
	elseif (PlayerClass == 'ROGUE' or PlayerClass == 'DRUID') then
		...
		if (PlayerClass == 'DRUID') then
			...
			RequireSpell = isClassic and 768 or 5221 -- Cat form or Shred
		end
	elseif (isRetail and PlayerClass == 'MAGE') then
...
local function Enable(self, unit)
	...
	if (element and UnitIsUnit(unit, 'player')) then
		...
		if (isRetail and (RequireSpec or RequireSpell)) then
